...
import { PIXELMATCH } from '../../shared/compare-config';
...
async function diff(req: DiffRequest): Promise<DiffSuccess> {
    ...
    if (a.width !== b.width || a.height !== b.height) {
        return { id: req.id, ok: true, dimsMatch: false, changedRatio: 1, diffPixels: b.width * b.height, width: b.width, height: b.height, diffBitmap: null };
    }
    const { width, height } = a;
    const out = new Uint8ClampedArray(width * height * 4);
    const diffPixels = pixelmatch(a.data, b.data, out, width, height, PIXELMATCH);
    const diffBitmap = await createImageBitmap(new ImageData(out, width, height));
    return { id: req.id, ok: true, dimsMatch: true, changedRatio: diffPixels / (width * height), diffPixels, width, height, diffBitmap };
}
