...
export function ProvenanceBadge({ source }: { source: EntrySource }) {
    ...
    return (
        <span className="text-xs text-zinc-500" title={`Promoted from run ${source.runId} (${source.branch} @ ${source.commit.slice(0, 10)}) by ${source.promotedBy}`}>
            from run <span className="font-mono text-zinc-400">#{source.runId}</span> · {source.branch} ·{' '}
